...
function NowPlayingMovies() {
    ...
    useEffect(() => {
        const url = `https://api.themoviedb.org/3/movie/now_playing?api_key=14ccdb96456935bbb41591e99697d262&language=ru-RU`;
        setIsLoading(true);
        fetch(url)
            .then((response) => {
    ...
    return (
        <div>
            {!isError && (<><h2>Сейчас в кино</h2></>)}
            {isLoading && (<Spinner cName={'main-spinner'} />)}
